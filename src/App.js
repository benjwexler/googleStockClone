    ...
    switch (_selectedTab) {
      ...
      case 1:
        return (day) => {
          return {
            x: moment(day.date)
              .add(day.minute.slice(0, 2), "hours")
              .add(day.minute.slice(-2), "minutes"),
            y: day.close,
            minute: day.minute,
          };
        };
    ...
    const _fetch = async () => {
      ...
      try {
        const res = await axios.get(
          isRealData && selectedTab > 1 ? url : testUrl
        );
        ...
        const formattedData = data.map(
          getFormattedDataAction(selectedTab, data)
        );
        ...
        <ThemeProvider theme={theme}>
          <div>
            <WindowedSelect
              ...
              onChange={(stock) =>
                stock && stock.symbol ? setStock(stock) : null
              }
              className="select"
            />
          </div>
          <h3 className="stockName" style={{ marginLeft: 10 }}>
          ...
          <h2 style={{ marginLeft: 10 }}>
            ...
            >
              {" "}
              {(quote.latestPrice - quote.previousClose).toFixed(2)} (
              {getPercentageChange(quote.latestPrice, quote.previousClose)}%){" "}
            </span>
            <i
              style={{
                fontSize: 14,
